    ...
    @foreach ($this->installed as $extension)
        ...
        >
            ...
            <div class="mb-4">
                ...
                >
                    <x-slot name="heading">
                        Enable per egg extension control
                    </x-slot>
                    <x-slot name="headerEnd">
                        <x-filament::button
                                wire:click="setPerEggControl('{{ $extension['slug'] }}', false)"
                                :color="($extension['egg_settings']['enabled_for_all'] ?? true) ? 'gray' : 'primary'"
                                x-on:click.stop
                        >
                            Enabled
                        </x-filament::button>
                        ...
                        <x-filament::button
                                wire:click="setPerEggControl('{{ $extension['slug'] }}', true)"
                                :color="!($extension['egg_settings']['enabled_for_all'] ?? true) ? 'gray' : 'primary'"
                                x-on:click.stop
                        >
                            Disabled
                    ...
                    @if(!($extension['egg_settings']['enabled_for_all'] ?? true))
                        <x-filament::input.wrapper
                                prefix-icon="heroicon-c-magnifying-glass"
                                prefix-icon-color="black"
                                class="mb-2">
                            <x-filament::input
                                    ...
                                    x-data="{
                                            ...
                                            debounceSubmit() {
                                                clearTimeout(this.timer);
                                                this.timer = setTimeout(() => {
                                                    $wire.filterEggs('{{ $extension['slug'] }}');
                                                }, 500);
                                            }
                                        }"
                                    x-on:input="debounceSubmit()"
                        ...
                        <p class="text-gray-500">To see your desired egg, narrow down the search. If you dont know the name, you can look it up in 'Eggs' tab.</p>

                        <div class="overflow-x-auto rounded-xl shadow">
                            <table class="w-full divide-y divide-gray-200 dark:divide-white/5" style="width: 100%;">
                                <thead class="bg-gray-50 dark:bg-gray-800">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 dark:text-gray-300 uppercase" style="width: 1em;">ID</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 dark:text-gray-300 uppercase">Egg</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 dark:text-gray-300 uppercase" style="width: 150px;"></th>
                                </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-white/5">
                                @foreach (($extension['eggs'] ?? []) as $egg)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            {{ $egg['id'] }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if(in_array($egg['id'], $extension['egg_settings']['allowed_egg_ids']))
                                                <x-filament::button
                                                        wire:click="setEggForPermission('{{ $extension['slug'] }}', {{ $egg['id'] }}, false)"
                                                        color="danger"
                                                        size="sm"
                                                >
                                                    Remove
                                            ...
                                            @else
                                                <x-filament::button
                                                        wire:click="setEggForPermission('{{ $extension['slug'] }}', {{ $egg['id'] }}, true)"
                                                        size="sm"
                                                >
                                                    Add
                                                </x-filament::button>
